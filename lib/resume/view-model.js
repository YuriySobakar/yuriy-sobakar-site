...
//   - headlineLines: the headline split on " | " — the hero prints it as a plain column, one line each
...
export function buildViewModel(data) {
  const vm = {
    ...
    // The headline stays one string for <title> / meta; " | " separates the lines the hero shows as a column.
    headlineLines: String(data.headline ?? "").split(" | ").map((line) => line.trim()).filter(Boolean),
